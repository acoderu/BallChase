...
class SensorBuffer:
    ...
    def find_synchronized_measurements(self, min_sensors=1, primary_sensor=None):
        ...
        Optimized implementation with early returns and simplified calculations.
        ...
        """
        # Check if we have enough sensors with data - early return for efficiency
        sensors_with_data = []
        for s, b in self.buffers.items():
            if len(b) > 0:
                sensors_with_data.append(s)
                
                # Early exit optimization - if we only need one sensor, return first found
                if min_sensors == 1 and not primary_sensor:
                    return {s: b[-1][1]}  # Return most recent measurement
        
        if len(sensors_with_data) < min_sensors:
            return {}

        # Find sensor to use as reference time - prioritize important sensors
        if primary_sensor and primary_sensor in sensors_with_data:
            ref_sensor = primary_sensor
        else:
            # First try LiDAR as primary if available
            ...
            # Then try YOLO 3D
                ...
                ref_sensor = 'yolo_3d'
            # Then any 3D sensor
            elif any(s for s in sensors_with_data if not s.endswith('_2d')):
                ref_sensor = next(s for s in sensors_with_data if not s.endswith('_2d'))
            else:
                # Fall back to any sensor
                ref_sensor = sensors_with_data[0]
        
        # For secondary priority sensors, reduce search complexity
        if len(sensors_with_data) > 3 and ref_sensor in ['hsv_3d', 'hsv_2d']:
            # For less important sensors, just take most recent data from others
            result = {}
            result[ref_sensor] = self.buffers[ref_sensor][-1][1]  # Most recent measurement
            
            # Add most recent measurement from other sensors if fresh enough
            ref_time = self.buffers[ref_sensor][-1][0]
            for sensor in sensors_with_data:
                if sensor != ref_sensor:
                    most_recent = self.buffers[sensor][-1]
                    if abs(most_recent[0] - ref_time) <= self.default_max_time_diff * 1.5:
                        result[sensor] = most_recent[1]
            
            return result
        
        # For important sensors, do proper synchronization
        best_sync = {}
        best_score = 0.0
        
        # Limit search to 10 most recent measurements for performance
        ref_measurements = list(self.buffers[ref_sensor])[-10:]
        
        # For each measurement from reference sensor (starting from most recent)
        for i, (ref_time, ref_data) in enumerate(reversed(ref_measurements)):
            # Early exit if we're too far back in history
            if i > 5 and best_sync and len(best_sync) >= min_sensors:
                break
                
            current_sync = {ref_sensor: ref_data}
            current_score = 0.0
            
            # Keep track of how many sensors we've checked (for early exit)
            checked_sensors = 1
            ...
            for sensor in sensors_with_data:
                ...
                # Early skip for unimportant sensors if we already have enough
                if len(current_sync) >= min_sensors and sensor not in ['lidar', 'yolo_3d', 'yolo_2d']:
                    # Skip less important sensors
                    continue
                ...
                # Limit search to 5 most recent measurements for secondary sensors
                sensor_measurements = list(self.buffers[sensor])[-5:]
                
                for time_val, data in reversed(sensor_measurements):
                    ...
                    if time_diff < max_time_diff * 0.2:
                        break
                ...
                # Early exit if we've checked enough sensors
                if checked_sensors >= 3 and len(current_sync) >= min_sensors:
                    break
            ...
            # Early exit if we have a good enough match
            if len(best_sync) >= min(3, len(sensors_with_data)) and best_score > 0.8:
                break
        
        # If we didn't find any good synchronization
        if not best_sync and min_sensors == 1 and sensors_with_data:
            # Fall back to most recent measurement from reference sensor
            return {ref_sensor: self.buffers[ref_sensor][-1][1]}
...
class EnhancedFusionLifecycleNode(LifecycleNode):
    ...
    def detect_motion_state(self):
        """
        Balanced motion state detector with improved efficiency and reduced oscillation.
        ...
        # Get the most recent velocity estimates (limited to exactly 3 for efficiency)
        recent_velocities = list(self.velocity_history)[-3:]
        
        # Calculate the average magnitude of these velocities efficiently
        avg_speed = 0.0
        valid_count = 0
        ...
        # Initialize state counters if needed
        if not hasattr(self, 'state_counters'):
            self.state_counters = {
                "stationary": 0,
                "small_movement": 0,
                "medium_fast": 0
        ...
        # Define thresholds for state classification with better noise filtering
        stationary_thresh = 0.03    # Increased to filter sensor noise
        small_movement_thresh = 0.15
        
        # Determine base motion state from velocity
        if avg_speed < stationary_thresh:
            base_motion_state = "stationary"
            self.state_counters["stationary"] += 1
            self.state_counters["small_movement"] = 0
            self.state_counters["medium_fast"] = 0
        elif avg_speed < small_movement_thresh:
            base_motion_state = "small_movement"
            self.state_counters["stationary"] = 0
            self.state_counters["small_movement"] += 1
            self.state_counters["medium_fast"] = 0
        else:
            base_motion_state = "medium_fast"
            self.state_counters["stationary"] = 0
            self.state_counters["small_movement"] = 0
            self.state_counters["medium_fast"] += 1
        
        # Get current motion state for comparison
        current_motion_state = getattr(self, 'motion_state', 'unknown')
        
        # Apply hysteresis to prevent oscillation
        # Require more evidence to change states (2 consecutive detections)
        if current_motion_state == "stationary":
            # Exiting stationary requires consistent evidence
            if base_motion_state != "stationary" and self.state_counters[base_motion_state] >= 2:
                ...
                # Initialize or continue tracking stationary time
                if not hasattr(self, 'stationary_start_time') or self.stationary_start_time is None:
                    self.stationary_start_time = current_time
                
                # Check for long-term stationary (reduced threshold to 5 seconds)
                stationary_duration = current_time - self.stationary_start_time
                if stationary_duration > 5.0:
                    motion_state = "long_stationary"
                    # Track long_stationary duration
                    if not hasattr(self, 'long_stationary_since'):
                        self.long_stationary_since = current_time
                else:
                    motion_state = "stationary"
        elif current_motion_state == "long_stationary":
            # Require stronger evidence to exit long_stationary
            if base_motion_state != "stationary" and self.state_counters[base_motion_state] >= 3:
                motion_state = base_motion_state
            ...
            # For movement states, use different hysteresis
            if base_motion_state == "stationary" and self.state_counters["stationary"] >= 3:
                # Entering stationary requires consistent evidence
                motion_state = "stationary"
                self.stationary_start_time = current_time
            else:
                # Staying in or transitioning between movement states
                # Only change movement states with consistent evidence (2 cycles)
                if base_motion_state != current_motion_state and self.state_counters[base_motion_state] >= 2:
                    motion_state = base_motion_state
                else:
                    motion_state = current_motion_state
        
        # Log only if motion state changes to reduce log volume
        if motion_state != current_motion_state:
            self.prev_motion_state = current_motion_state
            ...
            # Log with improved formatting including duration, but less verbose
            elapsed_time = current_time - self.start_time
            duration_text = f" after {state_duration:.1f}s" if state_duration > 0.5 else ""
            self.get_logger().info(
                f"🔄 [T+{elapsed_time:.1f}s][STATE] {self.prev_motion_state} → {self.motion_state}{duration_text} "
                f"(v={avg_speed:.3f}m/s)"
            )
        else:
            # Store current state without logging
            self.motion_state = motion_state
    ...
    def get_adaptive_validation_threshold(self, sensor, innovation=None):
        """
        Optimized validation threshold calculation with reasonable caps to prevent excessive values.
        ...
        # Apply motion multiplier with upper bound
        threshold = base_threshold * motion_multipliers.get(motion_state, 1.8)
        ...
        if hasattr(self, 'prev_motion_state') and hasattr(self, 'motion_state'):
            if self.motion_state != self.prev_motion_state:
                ...
                # Consider a transition active for 3 seconds
                if time.time() - self.last_state_transition_time < 3.0:
                    ...
                    self.motion_state not in ["stationary", "long_stationary"]:
                        ...
                        threshold *= 2.0  # More moderate boost
                    else:
        ...
        if hasattr(self, 'consecutive_rejections_per_sensor'):
            ...
            if rejections > 2:  # Require at least 2 rejections
                # Calculate reasonable boost factor
                rejection_boost = min(2.0, 1.0 + (rejections * 0.2))
                threshold += base_threshold * (rejection_boost - 1.0)  # Additive boost instead of multiplicative
                
                # Log only for significant boosts to reduce log volume
                if rejections >= 3 and self.debug_level >= 1:
                    self.get_logger().info(
                        f"Applying rejection threshold boost for {sensor}: "
                        f"{rejection_boost:.2f}x → {threshold:.2f}"
                    )
        
        # Apply safety limits with reasonable caps
        max_safe_threshold = 100.0
        threshold = min(threshold, max_safe_threshold)
    ...
    def transform_point(self, point_msg, target_frame, is_2d=False):
        """
        Highly optimized transform point method for better performance on Pi.
        ...
        if not self.transform_available:
            # Reduce log spam by limiting transform warnings
            if not hasattr(self, '_transform_warning_throttle'):
                self._transform_warning_throttle = time.time()
                self.get_logger().warn(f"Transform not available - cannot transform point")
            elif time.time() - self._transform_warning_throttle > 10.0:
                self._transform_warning_throttle = time.time()
                self.get_logger().warn(f"Transform not available - cannot transform point")
            return None
        ...
        try:
            ...
            # Use cached transforms for common static relationships
            transform = None
            if point_msg.header.frame_id == 'ascamera_color_0' and target_frame == self.reference_frame:
                # Use cached camera to base transform
                transform = self.tf_camera_to_base
            elif point_msg.header.frame_id == 'lidar_frame' and target_frame == self.reference_frame:
                # Use cached lidar to base transform
                transform = self.tf_lidar_to_base
            
            # Only lookup transform if cache miss
            if transform is None:
                transform = self.tf_buffer.lookup_transform(
                    target_frame,
                    point_msg.header.frame_id,
                    rclpy.time.Time(),
                    rclpy.duration.Duration(seconds=0.05)  # Reduced timeout
                )
            
            # Optimize transform operations for performance
            if is_2d:
                # Make a shallow copy of the point to avoid modifying the original
                result = PointStamped()
                result.header = point_msg.header
                result.header.frame_id = target_frame
                
                # Preserve confidence in z for 2D points
                confidence = point_msg.point.z
                
                # Apply transform to x,y coordinates directly
                p = [point_msg.point.x, point_msg.point.y, 0.0, 1.0]  # Homogeneous coordinates
                ...
                # Convert quaternion to rotation matrix elements (just the essential parts)
                xx = qx * qx
                xy = qx * qy
                xz = qx * qz
                xw = qx * qw
                ...
                # Only compute matrix elements we need
                r00 = 1 - 2 * (yy + zz)
                r01 = 2 * (xy - zw)
                r10 = 2 * (xy + zw)
                r11 = 1 - 2 * (xx + zz)
                
                # Apply transform manually (faster than do_transform_point)
                result.point.x = r00 * p[0] + r01 * p[1] + tx
                result.point.y = r10 * p[0] + r11 * p[1] + ty
                result.point.z = confidence  # Restore confidence
                
                return result
            else:
                # For 3D points, use the standard transform
                return do_transform_point(point_msg, transform)
                
        except Exception as e:
            # Log errors less frequently to reduce overhead
            if not hasattr(self, '_transform_error_throttle'):
                self._transform_error_throttle = time.time()
                self._transform_error_count = 1
                if self.debug_level >= 1:
                    self.get_logger().warn(f"Transform error: {str(e)}")
            elif time.time() - self._transform_error_throttle > 10.0:
                if self.debug_level >= 1:
                    self.get_logger().warn(f"Transform error: {str(e)} (occurred {self._transform_error_count} times)")
                self._transform_error_throttle = time.time()
                self._transform_error_count = 1
            else:
                self._transform_error_count += 1
    ...
    def predict_state(self, dt):
        """
        Optimized state prediction with balanced friction and noise models.
        
        Args:
            dt (float): Time step in seconds
        """
        # Reset the state transition matrix to identity - 4D state vector
        if not hasattr(self, '_F_matrix_cache'):
            self._F_matrix_cache = {}
        
        # Use cached F matrix for common dt values to avoid repeated calculations
        dt_key = round(dt * 100)  # Round to nearest 0.01 second
        if dt_key in self._F_matrix_cache:
            self._F_matrix = self._F_matrix_cache[dt_key]
        else:
            # Only update matrix if not in cache
            self._F_matrix = np.eye(4, dtype=np.float32)
            self._F_matrix[0, 2] = dt  # x += vx*dt
            self._F_matrix[1, 3] = dt  # y += vy*dt
            # Cache the result for future use
            if len(self._F_matrix_cache) < 20:  # Limit cache size
                self._F_matrix_cache[dt_key] = self._F_matrix.copy()
        ...
        # Apply balanced motion state-based scaling
        motion_scale = 1.0  # Default scaling
        ...
        # Apply balanced transition boost
        if hasattr(self, 'prev_motion_state') and hasattr(self, 'motion_state'):
            if self.motion_state != self.prev_motion_state:
                # If transitioning from stationary to movement
                if self.prev_motion_state in ["stationary", "long_stationary"]:
                    motion_scale *= 1.5  # Moderate boost for transitions
        
        # Process noise parameters with balanced scaling
        q_pos = self.process_noise_pos * dt * motion_scale
        q_vel = self.process_noise_vel * dt * motion_scale
        
        # Apply balanced rolling friction
        friction_coef = 0.01  # Balanced friction
        
        # Calculate current velocity magnitude once
        vx, vy = self.state[2], self.state[3]
        current_velocity = math.sqrt(vx*vx + vy*vy)
        
        # Apply friction only when non-zero velocity
        if current_velocity > 0.001:
            # Calculate friction deceleration: a = μg
            deceleration = friction_coef * 9.81  # μg in m/s²
        ...
        # Fill in the 4x4 process noise matrix with balanced values
        # Position variances
        self._Q_matrix[0, 0] = q_pos * dt * dt / 3.0  # x position variance
        self._Q_matrix[1, 1] = q_pos * dt * dt / 3.0  # y position variance
        ...
        # Position-velocity covariances
        self._Q_matrix[0, 2] = self._Q_matrix[2, 0] = q_pos * dt * dt / 2.0  # x position-velocity
        self._Q_matrix[1, 3] = self._Q_matrix[3, 1] = q_pos * dt * dt / 2.0  # y position-velocity
        
        # Apply minimum velocity only when needed based on update count and motion state
        if not hasattr(self, '_min_vel_counter'):
            self._min_vel_counter = 0
        
        self._min_vel_counter = (self._min_vel_counter + 1) % 5  # Apply only every 5th update
        
        if self._min_vel_counter == 0 and motion_state in ["small_movement", "medium_fast"]:
            # Check current velocity
            if current_velocity < 0.01:  # Only apply if very small
                # Apply a small minimum velocity 
                min_velocity = 0.015 if motion_state == "small_movement" else 0.02
                
                # Only log when actually applying, and only if debug is enabled
                if self.debug_level >= 2:
                    self.get_logger().debug(
                        f"Applying minimum velocity in {motion_state} state "
                        f"(current={current_velocity:.4f}m/s, setting to {min_velocity:.3f}m/s)"
                    )
                    
                # Scale existing velocity vector if non-zero
                if current_velocity > 0.0001:
                    scale = min_velocity / current_velocity
                    self.state[2] *= scale
                    self.state[3] *= scale
                else:
                    # Default direction if velocity is essentially zero
                    self.state[2] = min_velocity
                    self.state[3] = 0.0
    ...
    def handle_sensor_recovery_balanced(self):
        """
        Optimized sensor recovery handler with reduced logging.
        ...
        # Check for recovery only on most important sensors (LiDAR and YOLO)
        ...
        for sensor in priority_sensors:
            ...
            if self.sensor_gap_detection[sensor]['gap_detected']:
                ...
                if msg is not None and gap_duration < 0.5:  # Fresh data confirmed
                    ...
                    # Log recovery for significant gaps only
                    if total_gap > 1.0:
                        self.get_logger().info(
                            f"✨ [T+{elapsed_time:.1f}s][RECOVERY] {sensor} recovered after {total_gap:.1f}s gap"
                        )
                    
                    # Apply moderate covariance adjustment
                    adjustment_factor = min(2.0, 1.2 + total_gap * 0.3)  # More moderate scaling
                    
                    # Modify state uncertainty to allow faster state changes
                    if hasattr(self, 'covariance'):
                        # Apply boost to position and velocity uncertainty
                        ...
                        # Log substantial boosts only
                        if adjustment_factor > 1.5 and total_gap > 1.0:
                            self.get_logger().info(
                                f"👐 [T+{elapsed_time:.1f}s][RECOVERY] Boost: factor={adjustment_factor:.1f}, "
                                f"uncertainty={self.position_uncertainty:.3f}m"
                            )
                    
                    # Set up recovery boost period (shorter duration)
                    self.sensor_gap_detection[sensor]['recovery_boost_active'] = True
                    self.sensor_gap_detection[sensor]['recovery_boost_end'] = current_time + min(2.0, total_gap)
            ...
            # Check for new gaps (only log significant ones)
                ...
                if not self.sensor_gap_detection[sensor]['gap_detected']:
                    ...
                    self.sensor_gap_detection[sensor]['gap_level'] = 0.3
                    
                    # Log only for significant gaps and important sensors to reduce verbosity
                    if gap_duration > 1.0 and sensor in ['lidar', 'yolo_3d']:
        ...
        # Handle multiple sensor recovery with moderate boost
        if len(recovered_sensors) >= 2:
            ...
            if hasattr(self, 'covariance'):
                multi_recovery_factor = 1.8  # More moderate factor
                self.covariance[0:2, 0:2] *= multi_recovery_factor
                self.covariance[2:4, 2:4] *= multi_recovery_factor * 1.2
                
                # Update uncertainty metrics
                self.position_uncertainty = math.sqrt((self.covariance[0, 0] + self.covariance[1, 1]) / 2.0)
                self.velocity_uncertainty = math.sqrt((self.covariance[2, 2] + self.covariance[3, 3]) / 2.0)
                
                # Log the multi-sensor recovery
                self.get_logger().info(
    ...
    def update_state(self, measurements):
        """
        Optimized update_state method with improved efficiency and reduced logging.
        ...
        # Process high-priority sensors first
        priority_order = ['lidar', 'yolo_3d', 'hsv_3d', 'yolo_2d_est3d', 'hsv_2d_est3d', 'yolo_2d', 'hsv_2d']
        
        # Sort measurements by priority
        sorted_measurements = {}
        for sensor in priority_order:
            if sensor in measurements:
                sorted_measurements[sensor] = measurements[sensor]
        
        # Process any remaining sensors
        for sensor in measurements:
            if sensor not in sorted_measurements:
                sorted_measurements[sensor] = measurements[sensor]
        
        # For each measurement in the prioritized set
        for sensor, msg in sorted_measurements.items():
            # Skip if this sensor already provided a successful update this cycle
            if getattr(self, '_updated_sensors', {}).get(sensor, False):
                continue
            ...
            # Store the latest sensor for context in other methods
            self.latest_sensor_source = sensor
            
            # Check position limits (reduced operation)
            max_coord = 7.5
            if abs(transformed.point.x) > max_coord or abs(transformed.point.y) > max_coord:
                # Increment rejection counter with logging at reduced frequency
                self.consecutive_rejections_per_sensor[sensor] = self.consecutive_rejections_per_sensor.get(sensor, 0) + 1
                if self.consecutive_rejections_per_sensor[sensor] % 5 == 0 and self.debug_level >= 1:
                    self.get_logger().info(
            ...
            # Setup measurement and matrices - with type checking
            if sensor.endswith('_2d') or is_2d_derived:
                # 2D points and 2D-derived 3D estimates use 2-element measurement
                z = np.array([transformed.point.x, transformed.point.y], dtype=np.float32)
                H = self._H_matrix_2d  # Reuse matrix
            else:
                # 3D sensors - extract x,y components
                z = np.array([transformed.point.x, transformed.point.y], dtype=np.float32)
                # Use cached matrix with fixed components
                if not hasattr(self, '_H_matrix_2d'):
                    self._H_matrix_2d = np.zeros((2, 4), dtype=np.float32)
                    self._H_matrix_2d[0, 0] = 1.0  # Extract x position
                    self._H_matrix_2d[1, 1] = 1.0  # Extract y position
                H = self._H_matrix_2d
            
            # Set measurement noise based on sensor type (simplified)
            if sensor == 'lidar':
            ...
            try:
                ...
                # Get validation threshold with validation boost applied
                threshold = self.get_adaptive_validation_threshold(sensor, mahalanobis_dist)
                
                # Apply a moderate validation boost for stationary states
                if motion_state in ["stationary", "long_stationary"]:
                    threshold *= 1.3
                
                # Skip measurement if it fails validation
                if mahalanobis_dist > threshold:
                    # Log rejection at reduced frequency
                    self.consecutive_rejections_per_sensor[sensor] = self.consecutive_rejections_per_sensor.get(sensor, 0) + 1
                    
                    # Only log every 3rd rejection to reduce spam
                    if self.consecutive_rejections_per_sensor[sensor] % 3 == 0 and mahalanobis_dist > threshold * 1.5:
                        ...
                        self.get_logger().info(
                            ...
                            f"innovation {mahalanobis_dist:.2f} > threshold {threshold:.2f}, diff={pos_diff:.3f}m"
                        )
                ...
                if not hasattr(self, '_updated_sensors'):
                    self._updated_sensors = {}
                self._updated_sensors[sensor] = True
                
                # Update consecutive updates counter
                self.consecutive_updates += 1
                
            except np.linalg.LinAlgError:
                # Limit matrix inversion error logging
                if not hasattr(self, '_matrix_error_time'):
                    self._matrix_error_time = time.time()
                    self.get_logger().warn(f"⚠️ [T+{elapsed_time:.1f}s][MATRIX] Inversion failed for {sensor}")
                elif time.time() - self._matrix_error_time > 30.0:
                    self.get_logger().warn(f"⚠️ [T+{elapsed_time:.1f}s][MATRIX] Inversion failed for {sensor}")
                    self._matrix_error_time = time.time()
                continue
            ...
            # Kalman gain
            try:
                K = np.dot(np.dot(self.covariance, H.T), S_inv)  # Reuse S_inv
                ...
                # Apply balanced blending approach
                if motion_state in ["stationary", "long_stationary"] and pos_discrepancy > 0.08:
                    ...
                    # Log significant transitions only
                    if pos_discrepancy > 0.15:
                        self.get_logger().info(
                ...
                # Special case for moderate discrepancies > 10cm
                    ...
                    blend_factor = min(0.6, 0.5 + (pos_discrepancy / 2.0))
                    
                    # More trust for lidar
                    if sensor == 'lidar':
                        blend_factor = min(0.75, blend_factor + 0.15)
                    ...
                    # Log only substantial updates
                    if pos_discrepancy > 0.25 and self.debug_level >= 1:
                        self.get_logger().info(
                ...
                if pos_change > 0.1 and motion_state in ["stationary", "long_stationary"]:
                    self.get_logger().info(
                ...
                # Limit matrix error logging
                if not hasattr(self, '_kalman_error_time'):
                    self._kalman_error_time = time.time()
                    self.get_logger().warn(f"⚠️ [T+{elapsed_time:.1f}s][MATRIX] Kalman update failed for {sensor}")
                elif time.time() - self._kalman_error_time > 30.0:
                    self.get_logger().warn(f"⚠️ [T+{elapsed_time:.1f}s][MATRIX] Kalman update failed for {sensor}")
                    self._kalman_error_time = time.time()
                continue
        
        # Reset updated sensors tracking for next cycle
        self._updated_sensors = {}
    ...
    def adjust_covariance_for_gaps(self):
        """
        Balanced covariance adjustment with optimized calculations.
        """
        ...
        # Only check key sensors for efficiency
        key_sensors = ['lidar', 'yolo_3d', 'yolo_2d']
        for sensor in key_sensors:
            ...
            # Use simplified gap level calculation
            if gap_duration > 0.5:
                avg_gap_level += min(1.0, gap_duration / 3.0)
            elif self.consecutive_rejections_per_sensor.get(sensor, 0) >= 3:
                avg_gap_level += 0.3
        ...
        # Set moderate growth rate based on active sensors
        if active_sensors >= 2:
            growth_rate = 1.05 + (avg_gap_level * 0.1)
        elif active_sensors == 1:
            growth_rate = 1.1 + (avg_gap_level * 0.15)
        else:  # No effectively active sensors
            growth_rate = 1.2 + (avg_gap_level * 0.2)
        ...
        self.covariance[2:4, 2:4] *= growth_rate * 1.1  # Velocity uncertainty
        ...
        # Apply caps to prevent extreme uncertainty
        ...
        # Only log covariance growth at low frequency to reduce spam
        if not hasattr(self, '_covar_log_counter'):
            self._covar_log_counter = 0
        
        self._covar_log_counter = (self._covar_log_counter + 1) % 20  # Log every 20th update
        
        if self._covar_log_counter == 0 and avg_gap_level > 0.3:
            elapsed_time = current_time - self.start_time
            rejected_count = sum(1 for s, c in self.consecutive_rejections_per_sensor.items() 
                                if s in key_sensors and c > 2)
            
            self.get_logger().info(
                f"📈 [T+{elapsed_time:.1f}s][COVAR] Growth rate={growth_rate:.2f}, gaps={avg_gap_level:.1f}, "
                f"rejected={rejected_count}, uncertainty={current_pos_uncertainty:.3f}m"
            )

    def filter_update(self):
        """
        Optimized filter update with improved efficiency and reduced logging.
        """
        ...
        try:
            ...
            successful_update = False
            if measurements:
                # Only log synchronized sensors when multiple sensors and at low frequency
                if len(measurements) > 1 and hasattr(self, '_sync_log_counter'):
                    self._sync_log_counter = (self._sync_log_counter + 1) % 10
                    if self._sync_log_counter == 0 and self.debug_level >= 2:
                        elapsed_time = current_time - self.start_time
                        self.get_logger().debug(
                            f"🔄 [T+{elapsed_time:.1f}s][SYNC] Synchronized sensors: {', '.join(measurements.keys())}"
                        )
                else:
            ...
            # Store state in history buffers with reduced frequency
            if hasattr(self, 'position_history') and hasattr(self, 'velocity_history') and hasattr(self, 'time_history'):
            ...
            if not hasattr(self, 'uncertainty_counter'):
                self.uncertainty_counter = 0
            
            self.uncertainty_counter = (self.uncertainty_counter + 1) % 5
            if self.uncertainty_counter == 0:
            ...
            self.handle_sensor_recovery_balanced()
            
            # Update motion state on successful updates
            if successful_update:
                self.detect_motion_state()
            
            # Log performance metrics at reduced frequency (every 20 seconds)
            if not hasattr(self, '_perf_log_time'):
                self._perf_log_time = time.time() - 25.0  # Log first stats after 5 seconds
                
            if current_time - self._perf_log_time > 20.0:
                ...
                if HAS_PSUTIL:
                    try:
                        ...
                        # Count active sensors
                        active_count = sum(1 for s, t in self.last_detection_time.items() 
                                        if current_time - t < 1.0)
                        
                        elapsed_time = current_time - self.start_time
                        self.get_logger().info(
                            f"🖥️ [T+{elapsed_time:.1f}s][SYSTEM] CPU: {cpu_percent:.1f}%, Memory: {mem.percent:.1f}%, "
                            f"Active sensors: {active_count}"
                        )
                    except:
                        pass
    ...
    def publish_state(self):
        """
        Publish the current state estimate with improved efficiency and reduced logging.
        """
        # Skip if not active
        if not self.is_activated:
            return
        ...
        # Only log when position changes significantly (>10cm) or time threshold (1s) reached
        significant_change = pos_change > 0.1  # Increased from 0.05 to reduce logs
        time_threshold_reached = current_time - self.last_position_log_time > 1.0  # Increased from 0.5s
        ...
        if significant_change or time_threshold_reached:
            ...
            # Log with improved formatting but limit frequency
            elapsed_time = current_time - self.start_time
            self.get_logger().info(
                f"📍 [T+{elapsed_time:.1f}s][FUSION] dist={distance:.2f}m, dir={direction:.1f}°, "
                f"pos=({current_pos[0]:.2f}, {current_pos[1]:.2f}), "
        ...
        # Pass through GroundPositionFilter as second stage
        filtered_pos = self.ground_filter.update(current_pos, current_time)
        
        # Update our state with the filtered position to maintain consistency
        self.state[0:2] = filtered_pos[0:2]  # Update x,y position
        
        # Create position message (reuse if possible)
        if not hasattr(self, '_pos_msg'):
            self._pos_msg = PointStamped()
        
        self._pos_msg.header.stamp = self.get_clock().now().to_msg()
        self._pos_msg.header.frame_id = self.reference_frame
        self._pos_msg.point.x = float(filtered_pos[0])
        self._pos_msg.point.y = float(filtered_pos[1])
        self._pos_msg.point.z = float(filtered_pos[2])
        self.position_pub.publish(self._pos_msg)
        
        # Create velocity message (reuse if possible)
        if not hasattr(self, '_vel_msg'):
            self._vel_msg = TwistStamped()
        
        self._vel_msg.header.stamp = self.get_clock().now().to_msg()
        self._vel_msg.header.frame_id = self.reference_frame
        
        # Get velocity from ground filter
        ground_velocity = self.ground_filter.get_velocity()
        ground_speed = self.ground_filter.get_speed()
        
        # Use ground filter velocity if significant, otherwise use Kalman filter
        if ground_speed > 0.05:
            self._vel_msg.twist.linear.x = float(ground_velocity[0])
            self._vel_msg.twist.linear.y = float(ground_velocity[1])
        else:
            self._vel_msg.twist.linear.x = float(self.state[2])
            self._vel_msg.twist.linear.y = float(self.state[3])
            
        self._vel_msg.twist.linear.z = 0.0  # Zero vertical velocity for ground movement
        self.velocity_pub.publish(self._vel_msg)
    ...
    def get_validation_threshold_for_2d_derived(self, source, motion_state, bbox_age=0.0):
        ...
        with optimized calculations and reasonable thresholds.
        ...
        """
        # Start with reasonable base thresholds that account for uncertainty
        if source.startswith('yolo'):
            # YOLO-based estimates tend to be more reliable than HSV
            base_threshold = 25.0  # Reduced from 40+
        else:
            base_threshold = 30.0  # Reduced from 45+
        ...
        if hasattr(self, 'prev_motion_state') and hasattr(self, 'motion_state'):
            if self.prev_motion_state != self.motion_state:
                # During transitions, especially from stationary, be more permissive but not extreme
                if self.prev_motion_state in ["stationary", "long_stationary"]:
                    threshold *= 1.7  # Moderate boost during transition from stationary
                    
                    if self.debug_level >= 2:  # Reduce log level to debug
                        self.get_logger().debug(
        ...
        if hasattr(self, 'consecutive_rejections_per_sensor'):
            ...
            if rejections >= 2:
                # Calculate reasonable boost factor
                boost_factor = 1.0 + (min(rejections, 5) * 0.2)  # Cap at 5 rejections
                threshold += base_threshold * (boost_factor - 1.0)  # Additive rather than multiplicative
                
                # Only log significant boosts to reduce volume
                if boost_factor > 1.5 and self.debug_level >= 2:
                    self.get_logger().debug(
                        f"Applying rejection boost for {source}: "
                        f"{boost_factor:.2f}x → {threshold:.2f}"
                    )
        
        # Cap maximum thresholds at a reasonable value
        max_safe_threshold = 90.0  # Significantly reduced from previous values
        threshold = min(threshold, max_safe_threshold)
    ...
    def detect_and_override_stuck_state(self):
        """
        More balanced intervention mechanism that detects when system is stuck in a state.
        Maintains core logic but with optimized calculations and reduced logging.
        
        Returns:
            bool: True if an override was applied
        """
        ...
        # Skip frequent checks to reduce CPU usage - check only every 5th call
        if not hasattr(self, '_override_check_counter'):
            self._override_check_counter = 0
        
        self._override_check_counter = (self._override_check_counter + 1) % 5
        if self._override_check_counter != 0:
            return False
        
        # STEP 1: Check if we're potentially in a problematic state (stationary states)
        problematic_state = self.motion_state in ["long_stationary", "stationary"]
        
        # Only continue checking if in a potentially problematic state
        if not problematic_state:
            return False
        
        # STEP 2: Check for movement evidence from sensors
        sensor_evidence = []
        movement_detected = False
        total_discrepancy = 0.0
        active_sensors = 0
        
        # Use modest threshold for movement detection - 5cm instead of 2cm
        movement_threshold = 0.05
        
        # Only check main sensors to reduce CPU load
        for sensor in ['lidar', 'yolo_3d', 'yolo_2d']:
            sensor_msg = self.sensor_buffer.get_latest_measurement(sensor)
            if sensor_msg is None:
                continue
            
            # Check freshness with balanced window
            if current_time - self.last_detection_time.get(sensor, 0) > 1.5:
                continue
            ...
            # Check if this sensor shows movement
        ...
        avg_discrepancy = total_discrepancy / active_sensors if active_sensors > 0 else 0.0
        ...
        # Update counter if movement detected - require multiple consistent detections
        ...
        # Check cooldown - moderate 5 second delay between interventions
        if current_time - self.last_override_time < 5.0:
            self.override_cooldown = True
            return False
        else:
            self.override_cooldown = False
        
        # STEP 4: Apply intervention after consistent evidence - require 3 consecutive detections
        if self.movement_override_counter >= 3 and not self.override_cooldown:
            elapsed_time = current_time - self.start_time
            self.get_logger().info(
                f"🔄 [T+{elapsed_time:.1f}s][OVERRIDE] Detected {len(sensor_evidence)} sensors showing average discrepancy "
                f"of {avg_discrepancy:.3f}m while in {self.motion_state} state"
            )
            
            # STEP 5: Force state change with balanced approach
            self.prev_motion_state = self.motion_state
            
            # Set to small_movement instead of directly to medium_fast for smoother transition
            self.motion_state = "small_movement"
            
            # Update confidence values to high but not maximum
            if hasattr(self, 'motion_state_confidence'):
                self.motion_state_confidence["small_movement"] = 0.9
                self.motion_state_confidence["medium_fast"] = 0.7
            ...
            # STEP 6: Apply balanced covariance increase
            self.covariance[0:2, 0:2] *= 3.0  # Moderate position uncertainty increase
            self.covariance[2:4, 2:4] *= 5.0  # Larger velocity uncertainty increase
            
            # Update uncertainty metrics
            self.position_uncertainty = math.sqrt(np.trace(self.covariance[0:2, 0:2]) / 2.0)
            self.velocity_uncertainty = math.sqrt(np.trace(self.covariance[2:4, 2:4]) / 2.0)
            
            # STEP 7: Balanced incorporation of sensor data
            if sensor_evidence:
                # Find most discrepant sensor
                most_discrepant = max(sensor_evidence, key=lambda x: x['discrepancy'])
                
                # Apply 80% sensor data weight (balanced from 100%)
                pos = most_discrepant['position']
                
                # Blend position with current state
                blend_factor = 0.8
                self.state[0] = (1.0 - blend_factor) * self.state[0] + blend_factor * pos[0]
                self.state[1] = (1.0 - blend_factor) * self.state[1] + blend_factor * pos[1]
                
                # Calculate implied velocity with moderate value
                if self.last_update_time is not None:
                    dt = current_time - self.last_update_time
                    if dt > 0.001:
                        # Set modest non-zero velocity in detected direction
                        min_speed = 0.1  # Reduced from 0.2
                        
                        # Calculate direction to maintain
                        ...
                        if dist > 0.001:
                            ...
                            vx = (dx / dist) * min_speed
                            vy = (dy / dist) * min_speed
                        else:
                            # Default direction if too small
                            vx = min_speed
                            vy = 0.0
                        
                        # Set velocity with blend
                        self.state[2] = vx
                        self.state[3] = vy
            
            # STEP 8: Reset rejection counters for fresh start
            if hasattr(self, 'consecutive_rejections_per_sensor'):
                for sensor in self.consecutive_rejections_per_sensor:
                    self.consecutive_rejections_per_sensor[sensor] = 0
            ...
            self.get_logger().info(
                f"✅ [T+{elapsed_time:.1f}s][OVERRIDE] Applied balanced override: state={self.motion_state}, "
    ...
    def cache_static_transforms(self):
        """
        Enhanced transform caching to avoid repeated lookups during execution.
        Caches more transforms and stores them efficiently.
        """
        self.get_logger().info("Caching static transforms...")

        try:
            # Create a cache dictionary if not exists
            if not hasattr(self, 'transform_cache'):
                self.transform_cache = {}
            
            # Define all important frame pairs to cache
            frame_pairs = [
            ...
            # Cache each transform with longer timeout
            for source, target in frame_pairs:
                try:
                    ...
                    # Also cache as matrix for faster calculations
                    if not hasattr(self, 'transform_matrices'):
                    ...
                    # Store matrix
                    self.transform_matrices[cache_key] = matrix
                    
                    # Store common shortcut transforms for direct access
                    if source == 'ascamera_color_0' and target == self.reference_frame:
                        self.tf_camera_to_base = transform
                    elif source == 'lidar_frame' and target == self.reference_frame:
                        self.tf_lidar_to_base = transform
                    
                    self.get_logger().info(
                        f"Cached static transform: {source} → {target}: "
                        f"trans=({tx:.3f}, {ty:.3f}, {tz:.3f})"
                    )
                    
                except Exception as e:
                    self.get_logger().warn(f"Failed to cache transform {source} → {target}: {str(e)}")
    ...
    def init_sensor_synchronization(self):
        """
        Initialize sensor synchronization system with optimized buffer sizes and settings.
        """
        # Create sensor buffer with moderate time tolerance
        self.sensor_buffer = SensorBuffer(max_time_diff=0.3)  # Reduced from 0.5
        ...
        # Optimize buffer sizes based on importance
        buffer_sizes = {
            'lidar': 25,     # Highest priority
            'yolo_3d': 20,   
            'hsv_3d': 15,    # Reduced size
            'yolo_2d': 20,   
            'hsv_2d': 15     # Reduced size
        }
        
        # Add sensors to the buffer with optimized buffer sizes
        for sensor in self.expected_sensors:
            size = buffer_sizes.get(sensor, 15)  # Default to smaller size
        ...
        # Use smaller buffer for FPS calculation to save memory
        self.sensor_frame_times = {sensor: deque(maxlen=20) for sensor in self.expected_sensors}
        ...
        # Initialize sensor recovery tracking with reduced memory usage
        self.sensor_gap_detection = {}
        for sensor in ['lidar', 'yolo_3d', 'yolo_2d']:  # Track only key sensors
            self.sensor_gap_detection[sensor] = {
                'gap_detected': False,
                'gap_start_time': 0.0,
                'gap_level': 0.0,  # Track gap severity (0.0-1.0)
                'recent_gaps': deque(maxlen=3)  # Reduced from 5
            }
        
        # Smaller predictive buffer for position/velocity tracking
        self.position_prediction_buffer = deque(maxlen=5)  # Reduced from 10
        self.velocity_prediction_buffer = deque(maxlen=5)  # Reduced from 10
        
        self.get_logger().info("Sensor synchronization system initialized with optimized buffers")
        ...
        self.smoothed_state_estimator = SmoothedStateEstimator(window_size=3)  # Reduced from 5

    def sensor_callback(self, msg, source):
        """
        Optimized sensor callback with improved efficiency and reduced logging.
        
        Args:
            msg (PointStamped): The sensor message
            source (str): Source identifier (e.g., 'lidar', 'yolo_2d')
        """
        # Skip if not active yet
        if not self.is_activated:
            return
        ...
        try:
            ...
            # Track frame time for FPS calculation (optimized)
            if len(self.sensor_frame_times[source]) > 0:
                # Only calculate FPS when adding at least the second measurement
                prev_time = self.sensor_frame_times[source][-1]
                self.sensor_frame_times[source].append(current_time)
                
                # Update FPS with simple exponential smoothing
                if not hasattr(self, '_fps_alpha'):
                    self._fps_alpha = 0.3  # Smoothing factor
                
                new_fps = 1.0 / max(0.01, current_time - prev_time)
                old_fps = self.sensor_fps[source]
                self.sensor_fps[source] = old_fps * (1.0 - self._fps_alpha) + new_fps * self._fps_alpha
            else:
                # First measurement
                self.sensor_frame_times[source].append(current_time)
            ...
            # Handle gap recovery
            if source in self.sensor_gap_detection and self.sensor_gap_detection[source]['gap_detected']:
                # Update recovery info
                self.sensor_gap_detection[source]['gap_detected'] = False
                total_gap = current_time - self.sensor_gap_detection[source]['gap_start_time']
                
                # Only log substantial gaps to reduce volume
                if total_gap > 1.0:
                    elapsed_time = current_time - self.start_time
                    self.get_logger().info(
                        f"✨ [T+{elapsed_time:.1f}s][RECOVERY] {source} recovered after {total_gap:.1f}s gap"
                    )
            
            # If not initialized yet, try EARLY fast initialization without logging
            if not self.initialized:
                # First check if LiDAR measurement (prioritize)
                if source == 'lidar':
                    transformed = self.transform_point(msg, self.reference_frame, False)
                    if transformed:
                ...
                # If still not initialized, try with 2D measurement
            ...
            # For LiDAR data, log less frequently
            if source == 'lidar':
                ...
                # Log only every 5th LiDAR message to reduce volume
                if self._lidar_log_counter % 5 == 0:
                    transformed = self.transform_point(msg, self.reference_frame, False)
                    if transformed:
            ...
            if not hasattr(self, '_callback_error_time'):
                self._callback_error_time = time.time()
                elapsed_time = current_time - self.start_time
                self.get_logger().error(f"❌ [T+{elapsed_time:.1f}s][{source.upper()}] Error: {str(e)}")
            elif time.time() - self._callback_error_time > 10.0:
                elapsed_time = current_time - self.start_time
                self.get_logger().error(f"❌ [T+{elapsed_time:.1f}s][{source.upper()}] Error: {str(e)}")
                self._callback_error_time = time.time()
    ...
    def publish_status(self):
        """
        Publish and log brief status information with improved formatting and reduced frequency.
        """
        # Skip if not active
        if not self.is_activated:
            return
        ...
        if not hasattr(self, '_status_msg'):
            self._status_msg = Bool()
        
        self._status_msg.data = bool(self.tracking_reliable)
        self.status_pub.publish(self._status_msg)
        
        # Only log status at reduced frequency
        if not hasattr(self, '_status_log_timer'):
            self._status_log_timer = 0
        
        self._status_log_timer += 1
        if self._status_log_timer % 3 != 0:  # Log every 3rd call (3 seconds)
            return
        ...
        # Log status - only if the state changes or periodically
        ...
        # Check if status changed or time threshold reached (reduced frequency)
        if (current_state != self._last_status_state or 
                current_time - self._last_status_time > 10.0):  # Increased from 5.0
            ...
            # Log active sensor information only if changed
            if not hasattr(self, '_last_sensor_state'):
                self._last_sensor_state = ""
            
            # Build sensor state
            sensor_state = []
            for sensor in ['lidar', 'yolo_3d', 'yolo_2d']:  # Only track primary sensors
                if self.sensor_counts.get(sensor, 0) > 0:
                    ...
                    if delay < 5.0:  # Only include recently active sensors
                        ...
                        sensor_state.append(f"{sensor}: {count}, {delay:.1f}s ago, {fps:.1f}Hz")
            
            sensor_state_str = " | ".join(sensor_state)
            
            # Only log if sensor state changed
            if sensor_state_str != self._last_sensor_state and sensor_state:
                self.get_logger().info(f"📡 [T+{uptime:.1f}s][SENSORS] {sensor_state_str}")
                self._last_sensor_state = sensor_state_str

    def publish_diagnostics(self):
        """Optimized diagnostics publishing with reduced frequency and payload."""
        # Only publish full diagnostics at reduced frequency
        current_time = time.time()
        
        # Initialize tracking for last full diagnostics
        if not hasattr(self, 'last_full_diagnostics_time'):
            self.last_full_diagnostics_time = 0
        
        # Do full diagnostics every 15 seconds
        do_full_diagnostics = current_time - self.last_full_diagnostics_time > 15.0
        ...
        # Create a minimal diagnostics dictionary with only essential info
        # Only include active sensors to reduce payload size
        active_sensors = {}
        for sensor in ['lidar', 'yolo_3d', 'yolo_2d']:
        ...
        # Publish diagnostics message (reuse if possible)
        if not hasattr(self, '_diag_msg'):
            self._diag_msg = String()
        
        self._diag_msg.data = json.dumps(diag)
        self.diagnostics_pub.publish(self._diag_msg)
