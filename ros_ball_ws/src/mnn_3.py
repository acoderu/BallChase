...
def inference(model, img, precision, backend, thread, num_runs=1):
    config = {
        "precision": precision,
        "backend": backend,
        "numThread": thread
    ...
    ih, iw, _ = original_image.shape  # Original image height and width
    ...
    print(f"shape09....{image.shape}")
    # Expand dimensions to (1, H, W, C) and convert to MNN tensor format
    input_var = np.expand_dims(image, 0)
    print(f"shape0....{input_var.shape}")
    input_var = MNN.expr.convert(input_var, MNN.expr.NC4HW4)

    print(f"shape1....{input_var.shape}")
    ...
    # x0, y0, x1, y1 are the bounding box corners
    x0 = (cx - w * 0.5)   
    y0 = (cy - h * 0.5)   
    x1 = (cx + w * 0.5) 
    y1 = (cy + h * 0.5) 
    ...
    if result_ids.shape[0] > 0:
        ...
        cv2.imwrite("res_raw_size.jpg", original_image)
        print("Detection results saved to res_raw_size.jpg")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--thread", type=int, default=4, help="Number of threads to use for inference")
